...
function getMatrix(count: number, min: number, max: number) {
  const matrix = Array.from({ length: count }, () =>
    Array.from({ length: count }, () => getRandomNumber(min, max)),
  )

  return matrix
}

function getReplacementCount(row: number[]) {
  const initialValues = {
    prevSign: 0,
    count: 1,
    replacements: 0,
...
function printMatrix(matrix: number[][]) {
  ...
  matrix.forEach((row, i) => {
    ...
    console.log(
      `${star} [${rowStr}] | min positive: ${minPosStr} | replacements needed: ${repl}`,
    )
  })
}
